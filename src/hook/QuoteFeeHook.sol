...
/// @notice The bonding curve as a Uniswap v4 hook, charging its whole fee in the quote asset in
///         every swap shape. `LaunchHook` is the currency-of-arrival version this contract
///         replaces for new launches: read `docs/plans/2026-09-05-pool-curve-design.md` for the
///         curve arithmetic, which is unchanged, and see `_charge` below for what did change.
///
/// Three of the four properties `LaunchHook` holds are unchanged and are not repeated here: the
/// pool opens where the launch's own curve parameters said it would, only the launch's own
/// liquidity is ever in it, and the price never leaves the curve. The fourth is what this
/// contract exists to change:
///
/// **The fee is always the quote, never the launch token.** `LaunchHook` takes its cut out of
/// whichever side of a swap it is allowed to, which is the side v4 calls unspecified, and that
/// side is the launch token on an exact-input buy. A protocol whose treasury is meant to hold
/// only the quote asset then has to sell a token it received as a fee before it can spend the
/// proceeds, and a buyback that is meant to retire the launch token cannot tell its own purchase
/// apart from a fee that arrived already in that shape. `beforeSwapReturnDelta` lets a hook take
/// its cut out of the **specified** side as well, so the currency the fee lands in becomes a
/// choice rather than an accident of direction: this hook always chooses the quote. Charged in
/// `beforeSwap` when the quote is specified (an exact-input buy, an exact-output sell) and in
/// `afterSwap` when it is not (an exact-input sell, an exact-output buy), settled as an ERC-6909
/// claim inside the PoolManager exactly as `LaunchHook.afterSwap` already does, so a trade still
/// moves no token to any address this hook does not control.
contract QuoteFeeHook is ILaunchHook, IUnlockCallback, Ownable2Step, ReentrancyGuard {
    ...
    /// Where an opening tax taken in the launch token goes. Claiming it performs the burn. Kept
    /// for a launch that still opens with a token-denominated snipe rate; this hook's own fee and
    /// creator tax never land here, because neither is ever taken in the launch token.
    ...
    /// The pull-escrow ledger. Every key here is the quote: this hook never charges its fee, its
    /// creator tax or its opening tax in the launch token, so `owed[token]` for a launch token is
    /// always empty.
    ...
    /// The permission bitmap, and the low 14 bits the hook's address must carry: `0x2ACC`, the
    /// same 6 bits `LaunchHook` carries at `0x2AC4` plus `beforeSwapReturnDelta`. A different
    /// permission bitmap is a different address, which is why this is a second hook rather than
    /// an upgrade to the first: `LaunchHook.sol`'s own doc explains why a live pool cannot move.
    ///
    ...
    /// keep every LP but the launch's locker out, permanently. `beforeSwap` refuses a trade
    /// before the launch's liquidity is in place and now also charges the fee on the two swap
    /// shapes where the quote is the specified side; `afterSwap` charges it on the other two,
    /// where the quote is unspecified. Both return deltas rather than reverting or transferring,
    /// which is why both return-delta bits are set.
    ...
    ///      refused, for the same three reasons `LaunchHook.beforeSwap` gives.
    ///
    ///      No longer `view`: on an exact-in buy and an exact-out sell, the quote is the swap's
    ///      **specified** side, and `beforeSwapReturnDelta` is the only way a hook may take a cut
    ///      of that side. It reduces the amount the pool actually swaps (an exact-in buy) or
    ///      raises the amount the pool must pay out (an exact-out sell) by the fee, and mints the
    ///      difference to itself as an ERC-6909 claim exactly the way `afterSwap` already does for
    ///      the other two shapes. `_charge` runs the same tax-then-fee-then-creator-tax split
    ///      either way; only which hook callback calls it, and on what side, has moved.
    ///
    ///      The exact-output-buy refusal below is unchanged from `LaunchHook`, and deliberately
    ///      so. It used to be the only way to avoid taxing an exact-output buy's quote leg at a
    ///      rate the curve's nonlinearity made worth many times an exact-input buy's; that reason
    ///      is gone once the tax can be taken from the quote on every shape, but lifting the
    ///      refusal is a separate change from moving the fee, and this one keeps it in place.
    function beforeSwap(
        address sender,
        PoolKey calldata key,
        ...
        return (IHooks.beforeSwap.selector, _chargeSpecifiedQuote(poolId, key, params, sender), 0);
    }

    /// The fee on the two swap shapes where the quote is the swap's specified side: an exact-in
    /// buy and an exact-out sell. v4 flips which side is specified on an exact-output swap, so
    /// testing for it here is the same test `_charge` always ran, read one step earlier. Returns
    /// `ZERO_DELTA` on the other two shapes, which `afterSwap` charges instead.
    ///
    /// Split out of `beforeSwap` itself, which already carries the phase and tax-window checks:
    /// combined into one function the locals below leave solc no way to lay out the stack.
    function _chargeSpecifiedQuote(
        ...
        if ((exactInput == params.zeroForOne) != quoteIsCurrency0) {
            return BeforeSwapDeltaLibrary.ZERO_DELTA;
        }

        uint256 grossQuote = uint256(exactInput ? -params.amountSpecified : params.amountSpecified);
        uint256 total = _charge(
            poolId,
            sender,
            Currency.wrap(config.quote),
            params.zeroForOne == quoteIsCurrency0,
            grossQuote
        );
        if (total == 0) return BeforeSwapDeltaLibrary.ZERO_DELTA;
        // Positive on both shapes: an exact-in buy's `amountToSwap` moves from `-grossQuote`
        // toward zero (the pool swaps less), and an exact-out sell's moves further from zero
        // (the pool must pay out more, and keeps the difference). v4 checks the result cannot
        // cross zero, so a fee that would consume the whole specified amount reverts there
        // rather than silently flipping the trade's direction.
        return toBeforeSwapDelta(total.toInt128(), 0);
    }
    ...
    ///
    /// It is the test an exact-output buy is refused on. `LaunchHook` keeps this refusal because
    /// a hook may take its afterSwap delta only out of the swap's unspecified side, which put the
    /// opening tax on the tokens an exact-input buy receives and on the quote an exact-output buy
    /// pays, and the same rate on those two legs is not the same penalty on a nonlinear curve.
    /// This hook can charge the quote on either shape, which removes that asymmetry, but lifting
    /// the refusal on the strength of it is a separate change: this hook keeps refusing an
    /// exact-output buy for as long as the tax would bite, unchanged from today. A sell is never
    ...
    /// @dev The fee and the opening tax, taken out of the swap's quote side always, plus the
    ///      range guard. `beforeSwap` already took them on an exact-in buy and an exact-out sell;
    ///      this only fires on the other two shapes, where the quote is the swap's unspecified
    ///      side and `afterSwapReturnDelta` is what lets a hook take it. The tax is charged first
    ///      and the fee on what is left, which is the order the curve rail used and the order
    ///      `beforeSwap` applies too.
    function afterSwap(
        address sender,
        PoolKey calldata key,
        IPoolManager.SwapParams calldata params,
        ...
        return
            (IHooks.afterSwap.selector, _chargeUnspecifiedQuote(poolId, key, params, delta, sender));
    }

    /// The fee on the two swap shapes where the quote is the swap's unspecified side: an
    /// exact-in sell and an exact-out buy. The other two are `beforeSwap`'s, and charging them
    /// again here would tax one trade twice, in two currencies, so this only fires when the
    /// unspecified side (whatever it turns out to be) is the quote; the launch-token case
    /// returns zero and leaves the trade alone.
    ///
    /// Split out of `afterSwap` itself for the same reason `_chargeSpecifiedQuote` is split out
    /// of `beforeSwap`: combined into one function the locals leave solc no way to lay out the
    /// stack.
    function _chargeUnspecifiedQuote(
        ...
        (bool isCurrency1, uint256 amount) = HookFeeMath.unspecified(
            delta, params.amountSpecified, params.zeroForOne
        );
        if (amount == 0) return 0;

        LaunchConfig storage config = _configs[poolId];
        bool quoteIsCurrency0 = Currency.unwrap(key.currency0) == config.quote;
        if (isCurrency1 == quoteIsCurrency0) return 0;
        ...
        return total.toInt128();
    }

    /// The whole of what a swap owes the launch, in the quote: the opening tax first, then the
    /// trade fee and the creator's tax on what is left, which is the order the curve rail applied.
    /// The fee and the tax are charged on the same base, so a creator moving their slider changes
    /// what a trader pays and nothing else.
    ///
    /// Called from `beforeSwap` on the quote's specified shapes and from `afterSwap` on its
    /// unspecified ones, with the same four arguments either way: `_charge` and `_settle` do not
    /// know or care which callback reached them, because a launch's payout is the same figure
    /// whichever side of the swap the currency happened to be moving on.
    ...
    /// for the same answer twice. `currencyToken` is always the quote on this hook, so the
    /// `BURN_SINK` branch below never fires in practice; it stays because `_settle` is otherwise
    /// identical to `LaunchHook`'s and a launch-token-denominated tax is `InvalidConfig` to
    /// register, not something this function should assume can never reach it.
    ///
